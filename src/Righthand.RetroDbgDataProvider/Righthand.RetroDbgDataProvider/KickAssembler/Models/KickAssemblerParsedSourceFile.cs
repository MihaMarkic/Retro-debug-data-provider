...
public partial class KickAssemblerParsedSourceFile : ParsedSourceFile
{
    ...
    internal static IsCursorWithinArrayKeywordResult? IsCursorWithinArrayKeyword(string text, int lineStart, int lineLength,
        int cursor)
    { 
    ...
    internal static CompletionOption? GetFileSuggestionInArrayKeywordCompletionOption(ReadOnlySpan<IToken> tokens,
        string text, int lineStart, int lineLength, TextChangeTrigger trigger, int column)
    {
        var line = text.AsSpan()[lineStart..(lineStart + lineLength)];
        if (line.Length == 0)
